...
import modalConfigInfo from './deviceModal/modalConfigInfo';
...
class PageComponent extends Component {
    ...
    if (extData.position.index === 1) {
      return (
        <div>
          ...
          >
            {/*{extData.position.devname}大屏*/}
          </div>
          <span className="icon iconfont icon-a-zu1896" style={{ color: '#22B3C4', fontSize: '24px' }} />
  ...
  renderMarkerDefaultHover(extData) {
    return (
      <div>
        ...
        >
          <Markhover type='screen' lightdev={extData.position.deveui} lightdevname={extData.position.devname} />
        </div>
        <span className="icon iconfont icon-a-zu1896" style={{ color: '#22B3C4', fontSize: '24px' }} />
        {/* <img src={ltIcon} style={{ width: '20px' }} /> */}
      </div>
  ...
  render() {
    const {
      treeData, treeDataArry,mapCenter, baseData, size,currentLight
    } = this.state;
    ...
    return (
      <Layout name="visualCenter">
        ...
        >
          ...
          {/* 信息发布 */}
          <MsgDelivery baseData={baseData} currentLight={currentLight} />
