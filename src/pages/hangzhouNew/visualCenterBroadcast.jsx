...
import modalConfigBroadcast from './deviceModal/modalConfigBroadcast';
...
class PageComponent extends Component {
    ...
    if (extData.position.index === 1) {
      return (
        <div>
          ...
          >
            {/*{extData.position.devname}广播*/}
          </div>
          <span className="icon iconfont icon-a-lujing2199" style={{ color: '#22B3C4', fontSize: '24px' }} />
  ...
  renderMarkerDefaultHover(extData) {
    return (
      <div>
        ...
        >
          <Markhover type='broadcast' lightdev={extData.position.deveui} lightdevname={extData.position.devname} />
        </div>
        <span className="icon iconfont icon-a-lujing2199" style={{ color: '#22B3C4', fontSize: '24px' }} />
        {/* <img src={ltIcon} style={{ width: '20px' }} /> */}
      </div>
  ...
  render() {
    const { treeData, mapCenter, size, treeDataArry } = this.state;
